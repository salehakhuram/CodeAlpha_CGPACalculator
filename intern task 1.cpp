...
int main()
{
    ...
    do {
        ...
        cin >> sem;
        if(sem <= 0) cout << "Invalid! Semesters must be greater than 0.\n";
    } while(sem <= 0);
    ...
    do {
        ...
        cin >> course;
        if(course <= 0) cout << "Invalid! Number of courses must be greater than 0.\n";
    } while(course <= 0);
    ...
    for(int i = 0; i < course; i++)
    {
        ...
        float maxGrade;
        if(creditHours[i] == 3) maxGrade = 12;
        else if(creditHours[i] == 2) maxGrade = 8;
        else maxGrade = 4;

        // Grade Validation
        do {
            cout << "Enter grade for Course " << i+1 << " (0 - " << maxGrade << "): ";
            cin >> grade[i];
            if(grade[i] < 0 || grade[i] > maxGrade)
                cout << "Invalid! Grade must be between 0 and " << maxGrade << ".\n";
        } while(grade[i] < 0 || grade[i] > maxGrade);

        // Scale grade to 4.0 system
        double scaledGrade = (grade[i] / maxGrade) * 4.0;

        totalCredits += creditHours[i];
        totalGradePoints += scaledGrade * creditHours[i];
    }
    ...
    do {
        ...
        cin >> prevCredits;
        if(prevCredits < 0) cout << "Invalid! Credit hours cannot be negative.\n";
    } while(prevCredits < 0);

    double overallCGPA = 
        (prevCGPA * prevCredits + totalGradePoints) / (prevCredits + totalCredits);
